...
const TeacherDetail = () => {
  ...
  useEffect(() => {
    window.electronAPI.invoke('teachers:getById', id).then(data => {
      if (data) setTeacher(data)
      else toast.error('Teacher not found')
    })
  }, [id])
  ...
  if (!teacher) {
    return <div className="text-center text-red-600 mt-8">Teacher not found.</div>
  }
      ...
      <div className="space-y-6 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 rounded-md shadow">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Teacher Details</h1>
          </div>
          <div className="flex space-x-3">
            <Link
              to={`/app/teachers/${id}/edit`}
            ...
            <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow p-6">
              <div className="flex flex-col items-center">
                <img
                       src={
                          teacher.photoPath
                        ? `file://${teacher.photoPath}` // ✅ Convert absolute path to file URL
                ...
                <p className="text-gray-500 dark:text-gray-300">{teacher.subject}</p>
                <span
              ...
              <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div className="space-y-4">
                  ...
                  <InfoItem 
                    ...
                    label="Date of Birth" 
                    value={new Date(teacher.dob).toLocaleDateString('en-PK')} 
                  />
                  <InfoItem 
          ...
          <div className="lg:col-span-3">
            <div className="border-b border-gray-200">
              <nav className="-mb-px flex space-x-8">
                {['profile', 'classes', 'attendance', 'salary'].map(tab => (
                  <button
                    key={tab}
            ...
            <div className="mt-6">
              {activeTab === 'profile' && (
                <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow p-6">
                  ...
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <InfoBlock title="Subject Specialization" value={teacher.subject} />
                    <InfoBlock title="CNIC" value={teacher.cnic} />
                    <InfoBlock title="Emergency Contact" value={teacher.emergencyContact} />
                    <InfoBlock title="Joining Date" value={new Date(teacher.joiningDate).toLocaleDateString('en-PK')} />
                    <InfoBlock title="City" value={teacher.city} />
                  </div>

                  <h3 className="text-lg font-medium text-gray-900 dark:text-white mt-8 mb-4">Bank Details</h3>
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <InfoBlock title="Bank Name" value={teacher.bankName || 'Not specified'} />
                    <InfoBlock title="Account Number" value={teacher.accountNumber || 'Not specified'} />
                  </div>
                </div>
              )}
              ...
              {activeTab === 'classes' && (
                <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow p-6">
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white">Assigned Classes</h3>
                  <p className="mt-2 text-gray-500 dark:text-gray-400">Class assignment data will appear here</p>
                </div>
              )}
              ...
              {activeTab === 'attendance' && (
                <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow p-6">
                  ...
                  <p className="mt-2 text-gray-500 dark:text-gray-400">Attendance data will appear here</p>
                </div>
              )}
              ...
              {activeTab === 'salary' && (
                <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">Salary History</h3>
                    <div className="flex items-center space-x-2">
                      <CurrencyDollarIcon className="h-5 w-5 text-green-500" />
                      <span className="font-bold text-gray-900 dark:text-white">
                        Current Salary: PKR {teacher.salary?.toLocaleString() || '0'}
                      </span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <InfoBlock 
                      title="Last Payment" 
                      value={teacher.lastPaymentDate ? new Date(teacher.lastPaymentDate).toLocaleDateString('en-PK') : 'N/A'} 
                    />
                    <InfoBlock 
                      title="Payment Method" 
                      value={teacher.paymentMethod || 'Not specified'} 
                    />
                    <InfoBlock 
                      ...
                      value={
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {teacher.paymentStatus?.charAt(0).toUpperCase() + teacher.paymentStatus?.slice(1) || 'N/A'}
                        </span>
                      } 
                    />
                  </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded shadow max-w-md w-full">
            ...
            <p className="text-gray-700 dark:text-gray-300">Are you sure you want to delete this teacher?</p>
