...
useEffect(() => {
  ...
  window.electronAPI.invoke('students:list')
    .then(data => {
  ...
  {row.original.photo ? (
    <img
      ...
      onError={(e) => {
        ...
        e.target.src = `https://ui-avatars.com/api/?name=${row.original.name}&background=random`
      }}
    />
  ) : (
    <span>
    ...
    <div className="ml-4">
      ...
      <div
  className="text-gray-500 dark:text-gray-300 truncate max-w-[150px]"
  title={row.original.fatherName}
>
  ...
  Header: 'Class',
  accessor: 'classId',
  Cell: ({ value }) => <span>{classMap[value] || 'N/A'}</span>,
  Filter: ({ column }) => (
    <select
...
,
      ...
      },
      {
        Header: 'DOB',
        accessor: 'dob',
        Cell: ({ value }) => new Date(value).toLocaleDateString('en-PK')
      },
       {
        Header: 'Phone',
        accessor: 'phone',
  ...
  <div className="flex space-x-2">
    <button
      onClick={() => navigate(`/app/students/${row.original.id}`)}
    ...
    <button
      onClick={() => navigate(`/app/students/${row.original.id}/edit`)}
...
const data = useMemo(() => Array.isArray(students) ? students : [], [students])
...
;
  ...
  worksheet.getRow(2).eachCell(cell => {
    ...
    cell.fill = {
      ...
      degree: 0,
      stops: [
        { position: 0, color: { argb: 'FFD9E1F2' } },
        { position: 1, color: { argb: 'FFB4C6E7' } }
      ]
    };
  ...
  const row = worksheet.addRow({
    ...
    class: classMap[student.classId] || 'N/A',

    academicYear: student.academicYear,
    gender: student.gender,
    dob: new Date(student.dob).toLocaleDateString('en-PK'),
    phone: student.phone,
  ...
  worksheet.views = [
  { state: 'frozen', ySplit: 2 } // Freeze title + header
];
  ...
  if (student.status === 'Active') {
    statusCell.font = { color: { argb: 'FF006100' }, bold: true }; // green
  } else if (student.status === 'Inactive') {
    statusCell.font = { color: { argb: 'FF9C0006' }, bold: true }; // red
  }
});
  ...
  const timestamp = new Date().toISOString().split('T')[0]
   saveAs(blob, `Students_${timestamp}.xlsx`)
...
if (loading) {
  return (
    <div className="flex flex-col items-center justify-center h-[60vh] text-gray-600">
      ...
      <p className="text-sm">Loading students, please wait...</p>
    </div>
  )
}
  ...
  >
    ...
    >
      <Menu.Items
        className="origin-top-right absolute right-0 mt-2 w-72 max-h-[400px] overflow-y-auto overflow-x-hidden rounded-md shadow-lg bg-white dark:bg-gray-800 dark:text-gray-100 ring-1 ring-black ring-opacity-5 focus:outline-none z-[1000]"
      >
...
</div> {/* <-- This closes the new wrapping div */}
            ...
            <div>
              <p className="text-gray-700 dark:text-gray-300">
                ...
                <span className="font-medium">
                  {Math.min((pageIndex + 1) * pageSize, students.length)}
                </span>{' '}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div className="bg-white dark:bg-gray-800 p-6 rounded shadow max-w-md w-full">
      ...
      <p className="text-gray-700 dark:text-gray-300">Are you sure you want to delete this student?</p>
